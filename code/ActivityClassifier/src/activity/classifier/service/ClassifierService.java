...
public class ClassifierService extends Service  implements Runnable {
	...
	public void run() {
		...
		if(chargingState.equals("Charging")){
			...
			if (sd[0] < 3 * ssd[0] && sd[1] < 3 * ssd[1] && sd[2] < 3 * ssd[2] && !possiblyUncarried) {
				...
				if (possiblyUncarried) {
					Log.i("compare", "CompareX : " + (lastaverage[0] - 3 * ssd[0]) + " <= "
							+ average[0] + " <= " + (lastaverage[0] + 3 * ssd[0]));
					Log.i("compare", "CompareY : " + (lastaverage[1] - 3 * ssd[1]) + " <= "
							+ average[1] + " <= " + (lastaverage[1] + 3 * ssd[1]));
					Log.i("compare", "CompareZ : " + (lastaverage[2] - 3 * ssd[2]) + " <= "
							+ average[2] + " <= " + (lastaverage[2] + 3 * ssd[2]));
